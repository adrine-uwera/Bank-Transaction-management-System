...
from W7_A3_Q1_Adrine_Uwera_class_account import Account   # imports Account class from class_account file
...
# this function will allow the user to create an account, deposit money into an account, withdraw money from the account
# transfer money between accounts, view account details for an account and display all account details record of dangote
def main():
    print("\nDangote bank")
    print("------------")
        ...
        try:
            if start.upper() == 'Y':  # checks if the user wants to do a business operation
                ...
                if operation == '1':    # if user chooses to create account
                    print("\nCreate account")
                    print("--------------")
                    account_details = Account(name=input("Enter account holder's full name: "),
                                              age=input("Enter account holder's age in figures: "),
                                              gender=input("Enter account holder's gender (F/M): "),
                                              occupation=input("Enter account holder's occupation(if any): "))  # creates an instance of account class and allow the user to pass in account details
                    print("Account was created successfully!")  # message to indicate that the account was created
                    ...
                    for account in dangote_account_records:  # loops through the list of account details
                    ...
                    for sender_account in dangote_account_records:   # loops through the list of account details
                ...
                elif operation == "6":  # if user chooses to view details of an account
                    ...
                    for account in dangote_account_records:  # loops through the list of account details
                        ...
                        # displays to tell the user that the account referred is not found in dangote
                        print("\nAccount referred to does not exist under Dangote bank."
                              "\nPlease verify the account number entered and try again\n")

                elif operation == "7":  # if user chooses to view details of all accounts in dangote bank
